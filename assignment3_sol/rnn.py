...
class RNN_Model():
    ...
    def load_data(self):
        ...
        self.vocab = Vocab()
        train_sents = [t.get_words() for t in self.train_data]#获取train中单词
    ...
    def inference(self, tree, predict_only_root=False):
        ...
        if predict_only_root:#只对最后的输出节点做预测
            node_tensors = node_tensors[tree.root]
        else:
			#只读取标签不为2的node，即不为neutral的node，此时对每个节点都做预测
			#node代表合并后的词组，tensor代表对应的激活值
    ...
    def add_model(self, node):
		...
		#判断是否为根节点，如果是则拆分，不是则返回词向量
        if node.isLeaf:
            ### YOUR CODE HERE
            word_id = self.vocab.encode(node.word)
			#Gather slices from params according to indices.Produces an output tensor with shape indices.shape + params.shape[1:]
            # Scalar indices
