...
#input size is [batch,10],10代表的是对应单词的索引
#输入为10，意味着有10步，即timestep=10
#此练习根据输入的前N个单词来预测第N＋1个单词，给定一组字母比如a b c d e f
#如果input是a b c，那么target就是b c d，d就是想要预测的单词
#误差就是input与target的average log-perplexity per symbol
#训练网络最小化误差
...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
	  #len(self.vocab),number of different vocabulary
	...
	#inputs=[batch,10,50],split之后变为[batch,1,50],然后将1维删除
      inputs = [
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('RNN') as scope:
      ...
      for tstep, current_input in enumerate(inputs):
		...
		#rnn_outputs has 10 value,每个包含一组大小为[batch,D]的矩阵，即[10,B,D]
