...
#input size is [batch,3]，其中3代表的是输入单词的索引！
#然后从Embedding中找出每个索引对应的词向量
#送入神经网络并且优化参数
#预测
...
class NERModel(LanguageModel):
  ...
  def load_data(self, debug=False):
    ...
    # Load the training set, 3 words in a window
	#X_train contains the index of the corresponding input word rather than the original word
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
	  #这里用了默认的initializer初始化embedding
  ...
  def add_model(self, window):
    ...
    """
    ### YOUR CODE HERE
	#initializer: default initializer for variables within this scope.No need to write again
    with tf.variable_scope('Layer1', initializer=xavier_weight_init()) as scope:
      ...
      if self.config.l2:#Stores value in the collection with the given name.
  ...
  def __init__(self, config):
	...
	#input is 3*50 flattened vector
